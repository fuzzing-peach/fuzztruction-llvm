...
RUN --mount=type=cache,id=1,target=/var/cache/apt --mount=type=cache,target=/var/lib/apt \
    apt update -y \
    && apt install -y llvm-13-dev build-essential git cmake binutils-gold ninja-build ccache
...
RUN --mount=type=cache,target=/ccache/ \
    cmake -G Ninja -DCMAKE_BUILD_TYPE=RelWithDebInfo -DBUILD_SHARED_LIBS=on -DLLVM_USE_LINKER=gold \
        ...
        -DLLVM_ENABLE_DUMP=on -DLLVM_ENABLE_ZLIB=on -DLLVM_CCACHE_BUILD=on -DCMAKE_EXPORT_COMPILE_COMMANDS=ON ../llvm/llvm
